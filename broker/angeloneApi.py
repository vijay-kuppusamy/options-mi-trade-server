...
import base.optionsMiExceptions as ex
...
class SmartConnect(object):
    ...
    def __init__(self, user) -> None:
        self.user = user
        self.root = self._rootUrl
        self.api_key = user.tradingsession.apikey
        self.access_token = user.tradingsession.jwtToken
        self.refresh_token = user.tradingsession.refreshToken
        self.feed_token = user.tradingsession.feedToken
        self.userId = user.username
        self.status = user.tradingsession.status
        ...
        self.privateKey = user.tradingsession.apikey
    ...
    def login(self, clientCode, password, totp):
        ...
        if loginResultObject['status'] == True:
            ...
            self.user.tradingsession.jwtToken = "jwt4"
            self.user.tradingsession.refreshToken = "refresh4"
            self.user.tradingsession.feedToken = "feed4"
            self.user.save()

            profile = self.getProfile(refreshToken)
