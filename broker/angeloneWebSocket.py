...
class SmartWebSocket(object):
    ...
    def __init__(self, FEED_TOKEN, CLIENT_CODE, user):
    ...
    def constructTokens(self, symbols):
        token = ""
        if len(symbols) > 0:
    ...
    def run(self):
        while True:
            ...
            try:
                request = list(
                    SubscribeSymbol.objects.filter(user=self.user).values())
                if len(request) > 0:
                    # sub logic
                    tokens = self.constructTokens(request)
                    self.subscribe("mw", tokens)
                    SubscribeSymbol.objects.filter(user=self.user).delete()
